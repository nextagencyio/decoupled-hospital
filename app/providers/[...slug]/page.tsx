...
import { GET_PROVIDER_BY_PATH } from '@/lib/queries'
...
interface ProviderByPathData {
  route: {
    entity: DrupalProvider
  } | null
}

async function getProvider(path: string): Promise<DrupalProvider | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<ProviderByPathData>({
      query: GET_PROVIDER_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/providers/${slug.join('/')}`
  const item = await getProvider(path)

  if (!item) {
    return { title: 'Provider Not Found | Hospital' }
  }

  return {
    title: `${item.title} | Hospital`,
